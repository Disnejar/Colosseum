    ...
    namespace App
    {
        ...
        int Run()
        {
            ...
            while (!glfwWindowShouldClose(g_glfwWindow))
            {
                ...
                // Draw Each layer
                for (size_t i = 0; i < g_LayerStack.size(); i++)
                {
                    g_LayerStack[i]->DrawLayer();
                }
        ...
        {
            g_LayerStack.push_back(layer_ptr);
        }

        void RemoveLayer(std::shared_ptr<Layer> layer_ptr)
        {
            auto layerToRemove = std::find(g_LayerStack.begin(), g_LayerStack.end(), layer_ptr);

            if(layerToRemove != g_LayerStack.end()){
                g_LayerStack.erase(layerToRemove);
            }
        }
        ...
        void SetupVulkan(ImVector<const char*> instance_extensions)
        {
            ...
            // Create Vulkan Instance
            {
                ...
                debug_report_ci.pfnCallback = Utils::debug_report;
        ...
        {
            g_ResourceFreeQueue[g_CurrentFrameIndex].emplace_back(func);
        }
    }
}
