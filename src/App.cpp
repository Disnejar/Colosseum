...
namespace Colosseum{
    ...
    std::vector<std::shared_ptr<Layer>> App::_LayerStack {};
    ...
    void App::AddLayer(std::shared_ptr<Layer> layer_ptr)
    {
        _LayerStack.push_back(layer_ptr);
    }

    void App::RemoveLayer(std::shared_ptr<Layer> layer_ptr)
    {
        auto layerToRemove = std::find(_LayerStack.begin(), _LayerStack.end(), layer_ptr);

        if(layerToRemove != _LayerStack.end()){
            _LayerStack.erase(layerToRemove);
        }
    }
    ...
    VKAPI_ATTR VkBool32 VKAPI_CALL App::debug_report(VkDebugReportFlagsEXT flags, VkDebugReportObjectTypeEXT objectType, uint64_t object, size_t location, int32_t messageCode, const char* pLayerPrefix, const char* pMessage, void* pUserData)
    {
    ...
    bool App::IsExtensionAvailable(const ImVector<VkExtensionProperties>& properties, const char* extension)
    {
    ...
    VkPhysicalDevice App::SetupVulkan_SelectPhysicalDevice()
    {
        uint32_t gpu_count;
    ...
    void App::SetupVulkan(ImVector<const char*> instance_extensions)
    {
        VkResult err;
    ...
    void App::FrameRender(ImGui_ImplVulkanH_Window* wd, ImDrawData* draw_data)
    {
        VkResult err;
